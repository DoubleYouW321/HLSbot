...
TEST_1_QUESTIONS = [
    "В компании малознакомых людей вы скорее:",
    "После тяжелого дня вам больше поможет:",
    "Когда друг рассказывает вам о своей проблеме, вы:",
    "Для вас идеальный диалог — это:"
]

TEST_1_OPTIONS = [
    ["а) Включитесь в активный разговор, чтобы разрядить обстановку и познакомиться.",
     "б) Будете наблюдать со стороны, вступая в беседу, только если спросят или тема будет очень близка."],
    ...
    ["а) Часто ловите себя на мысли, что уже знаете, что ему посоветовать, и хотите это озвучить.",
     "б) В основном задаете уточняющие вопросы и киваете, давая ему выговориться."],
...
TEST_2_QUESTIONS = [
    "Источник энергии. Откуда у тебя появляется энергия после тяжелого дня или недели?",
...
TEST_2_OPTIONS = [
    ["а) От уединения. Мне нужен тихий вечер наедине с собой (книга, сериал, хобби), чтобы прийти в себя.",
     "б) От общения. Лучший отдых — встретиться с друзьями, сходить на вечеринку или в оживленное место. Одиночество утомляет."],
...
async def cmd_tests(callback: CallbackQuery):
    ...
    await callback.message.answer('''Выберите мини-тест, который вас интересует:

1. Ты слушатель или тот, кто говорит?
...
@mini_tests_router.callback_query(F.data.startswith('answer_'))
async def process_test_1_answer(callback: CallbackQuery, state: FSMContext):
    answer = callback.data.split('_')[1]  # 'a' или 'b'
    await callback.answer('')
...
async def finish_test_1(message: Message, state: FSMContext):
    ...
    if count_a > count_b:
        result = '''🎯 **Твои результаты:**

📊 Ответов А: {count_a}
📊 Ответов Б: {count_b}

**Преимущественно "ГОВОРИТЬ"**

Ты — тот, кто готов говорить. Ты оживляешь беседы, не даешь им заглохнуть, легко делишься идеями. 

💡 **Твоя зона роста** — давать больше пространства другим, практиковать активное слушание (переспрашивать, резюмировать), следить, не перебиваешь ли ты.'''.format(count_a=count_a, count_b=count_b)
...
Ты — Наблюдатель, Аналитик, Эмпат. Ты улавливаешь нюансы, тонкие эмоции, создаешь безопасное пространство для других, чтобы они раскрывались.

💡 **Твоя зона роста** — чаще делиться своим мнением, даже если оно не до конца сформировано, учиться вступать в разговор в шумной компании, ценить свой внутренний мир как не менее важный, чем мир других.'''.format(count_a=count_a, count_b=count_b)
...
**Примерно поровну (Сбалансированный тип)**
...
🎉 **Это очень ценный и сильный социальный навык!**'''.format(count_a=count_a, count_b=count_b)
...
🤝 Тебе комфортнее и ценнее иметь несколько по-настоящему близких и доверительных отношений, чем широкий круг поверхностных знакомств.'''.format(count_a=count_a, count_b=count_b)
...
🚀 **Твоя суперсила** — в коммуникабельности, инициативности и умении быстро адаптироваться!'''.format(count_a=count_a, count_b=count_b)
...
🔄 Тебе может нравиться быть в центре внимания на вечеринке, а на следующий день с таким же удовольствием ты проведешь время в полном уединении.
...
🌈 **Твоя суперсила** — в универсальности и умении находить общий язык с разными людьми!'''.format(count_a=count_a, count_b=count_b)
...
async def back_to_tests_menu(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.answer('')
    await callback.message.answer('Выберите мини-тест:', reply_markup=kb.tests_menu)