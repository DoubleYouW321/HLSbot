...
async def cmd_physics(callback: CallbackQuery):
    ...
    await callback.message.answer('В разделе Физическое благополучие ты можешь получить полезный совет из базы знаний, а также записывать и редактировать свои физические показатели:\n\n• Количество выпитых стаканов воды 💧\n• Часы сна 😴\n• Количество пройденных шагов 👣\n\nСравнивай их с нормой и следи за прогрессом!', reply_markup=kb.physics)
...
async def generate_advice(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(photo=photo, caption=advice, reply_markup=kb.back_to_physics)
...
async def cancel_input(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.answer("Ввод данных отменён. 🚫", reply_markup=kb.physics)
    await callback.answer()
