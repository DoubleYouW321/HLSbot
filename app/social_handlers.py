...
Шаг 1. Знай своего «врага»: Волнение — это просто энергия. Перенаправь её в голос (пусть он будет громче) и в жесты.

Шаг 2. Правило 10 секунд: Первые 10 секунд самые важные. Выучи наизусть ПЕРВУЮ фразу: «Здравствуйте, сегодня я расскажу о...». Сказал её — дальше пойдёт легче.
...
Шаг 5. Фишка от биоэтики: Представь, что ты не просто делишься фактами, а рассказываешь историю, которая может изменить чьё-то мнение. Это придаёт смысл. ✨''',
    2: '''Начни с малого. 👋
...
Помни: Большинство людей так же боятся показаться неинтересными. Твоё внимание — лучший подарок. 🎁''',
    3: '''Отказывать — это навык. 🛡️

1. Чётко и без извинений: «Нет, я не могу» звучит увереннее, чем «Ой, я не знаю, наверное, нет, извини...».
...
4. Фраза-щит от давления: Если продолжают уговаривать, повтори спокойно и твёрдо: «Я уже сказал(а) «нет».»

5. Философская основа: Уважение к своим границам — первый шаг к уважению себя. Тот, кто злится на твоё «нет», скорее всего, хотел воспользоваться твоим «да». 💪''',
}
...
async def cmd_social(callback: CallbackQuery):
    ...
    await callback.message.answer('''В разделе Социальное благополучие ты можешь:

🍅 Включить таймер Pomodoro для лучшей фокусировки и работоспособности
📝 Пройти мини-тесты, которые помогут тебе узнать многое о социальной стороне твоей личности
📚 Почитать гайды из библиотеки soft skills

Выбери интересующий тебя раздел:''', reply_markup=kb.social)
...
async def cmd_pomodoro_menu(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(photo=photo)
    await callback.message.answer('''🍅 Таймер Pomodoro — это инструмент в рамках техники тайм-менеджмента, который помогает сфокусироваться на задаче.

Как это работает:
• 25 минут работы ("помидор")
• 5 минут перерыва
• После 4 таких циклов — длинный перерыв 15 минут

Идеально для концентрации и продуктивности! ⏱️''', reply_markup=kb.pomodoro)
...
async def cmd_pomodoro_start(callback: CallbackQuery, bot: Bot):
    ...
    if user_id in pomodoro_timers and pomodoro_timers[user_id]['is_running']:
        await callback.message.answer('⚠️ У тебя уже есть запущенный таймер Pomodoro! Сначала останови текущий таймер.', reply_markup=kb.stop_timer)
        return
    ...
    await callback.message.answer('🍅 Таймер Pomodoro запущен!\n\n25 минут работы начались...\nСледи за уведомлениями о перерывах! ✅', reply_markup=kb.stop_timer)
...
async def pomodoro_worker(user_id: int, chat_id: int, bot: Bot):
    try:
        while user_id in pomodoro_timers and pomodoro_timers[user_id]['is_running']:
            ...
            await bot.send_message(chat_id=chat_id, text=f"🍅 25 минут работы начались...\n\nЦикл {pomodoro_timers[user_id]['cycles'] + 1}\nСосредоточься на задаче! ⏱️", reply_markup=kb.stop_timer)
            ...
            if cycles % 4 == 0 and cycles > 0:
                await bot.send_message(chat_id=chat_id, text=f"✅ Прошло 4 цикла!\n\n🍅🍅🍅🍅 Отличная работа!\nВремя для длинного перерыва - 15 минут...\nМожно прогуляться или сделать разминку 🏃‍♂️", reply_markup=kb.stop_timer)
                break_time = 15 * 60
            else:
                await bot.send_message(chat_id=chat_id, text=f"✅ Цикл {cycles + 1} завершён!\n\nВремя для перерыва - 5 минут...\nОтдохни, разомнись, посмотри в окно 👀", reply_markup=kb.stop_timer)
                break_time = 5 * 60
        ...
        try:
            await bot.send_message(chat_id=chat_id, text="⚠️ Произошла ошибка в работе таймера. Попробуй запустить заново.", reply_markup=kb.pomodoro)
        except:
            pass
...
async def cmd_pomodoro_stop(callback: CallbackQuery):
    ...
    if user_id in pomodoro_timers:
        ...
        await callback.message.answer(f'🛑 Таймер Pomodoro остановлен.\n\n✅ Выполнено циклов: {cycles}\n⏱️ Общее время работы: {cycles * 25} минут\n\nОтличная работа! Можешь запустить новый таймер, когда будешь готов! 🎯', reply_markup=kb.pomodoro)

        del pomodoro_timers[user_id]
    else:
        await callback.message.answer('ℹ️ У тебя нет запущенного таймера Pomodoro.', reply_markup=kb.pomodoro)
...
async def cmd_pomodoro_status(callback: CallbackQuery):
    ...
    if user_id in pomodoro_timers and pomodoro_timers[user_id]['is_running']:
        cycles = pomodoro_timers[user_id]['cycles']
        await callback.message.answer(f'📊 Статус Pomodoro\n\n✅ Выполнено циклов: {cycles}\n🔄 Текущий цикл: {cycles + 1}\n⏱️ Время работы: {cycles * 25} минут\n\nТаймер активен и работает... 🟢', reply_markup=kb.stop_timer)
    else:
        await callback.message.answer('📝 У тебя нет активного таймера Pomodoro.\n\nНажми "Запустить таймер", чтобы начать работу по технике Pomodoro.', reply_markup=kb.pomodoro)
...
async def cmd_lib(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(photo=photo)
    await callback.message.answer('''📚 В библиотеке soft skills ты можешь посмотреть интересные гайды по ситуациям в обществе, которые зачастую вызывают вопросы.

Выбери интересующий тебя гайд:
1. «Как уверенно выступить с докладом» 🎤
2. «Как завести разговор (даже если страшно)» 💬
3. «Как сказать «НЕТ» и не испортить отношения» 🛡️''', reply_markup=kb.gides)
